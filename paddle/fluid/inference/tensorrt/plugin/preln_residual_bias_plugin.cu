...
template <int UNROLL_FACTOR, int RESIDUAL_NUM>
...
#define HALF2_ADD_BIAS_RESIDUAL_LAYERNORM_OPT2(UNROLL_FACTOR)                \
  generalAddBiasResidualLayerNormOpt2<UNROLL_FACTOR, 1>                         \
      <<<rows, block, 0, stream>>>(reinterpret_cast<half2 *>(layernorm_dst), \
                                   ...
                                   half_n,                                   \
                                   epsilon);
#define HALF2_ADD_BIAS_LAYERNORM_OPT2(UNROLL_FACTOR)                        \
  generalAddBiasResidualLayerNormOpt2<UNROLL_FACTOR, 0>                         \
      <<<rows, block, 0, stream>>>(reinterpret_cast<half2 *>(layernorm_dst), \
...
int PrelnResidualBiasPluginDynamic::enqueue(
  ...
  VLOG(1)<<"@@@ preln residual bias plugin, residual_num:"<<residual_num_;
  ...
  if (input_type == nvinfer1::DataType::kFLOAT) {
    ...
    const int VecSize = 8;
    // if odd
    VLOG(1)<<"@@@ pin preln plugin configrue done in half";
    ...
    if (hidden % 2 == 0) {
      ...
      if(residual_num_==1){
      VLOG(1)<<"@@@ generalAddBiasResidualLayerNormOpt2<1>";
      ...
      switch (unroll_factor) {
        ...
        VLOG(1)<<"@@@ generalAddBiasResidualLayerNormOpt2<0>";
        switch (unroll_factor) {
          case 1:
            HALF2_ADD_BIAS_LAYERNORM_OPT2(1);
            break;
          case 2:
            HALF2_ADD_BIAS_LAYERNORM_OPT2(2);
            break;
          case 4:
            HALF2_ADD_BIAS_LAYERNORM_OPT2(4);
            break;
          case 8:
            HALF2_ADD_BIAS_LAYERNORM_OPT2(8);
            break;
          default:
            PADDLE_THROW(platform::errors::Fatal(
                "Invalid UNROLL_FACTOR in preln_residual_bias trt plugin."));
