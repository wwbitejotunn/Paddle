...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // Advance gV
        if (masking_step > 0) {
            if(n_block_max - n_block == (n_block_chunked_continus_num + 1)){
                tVgV.data() = tVgV.data() + n_block_sink_token_mask_num*(-int(kBlockN * params.v_row_stride));
            }
            tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
            if(threadIdx.x==0){
                printf("@#@#@#@ tvgv, q_seqlen_offset:%d, n_block_max:%d, n_block:%d,  n_block_chunked_continus_num:%d, n_block_sink_token_mask_num:%d, tVgV:%p \n ",
                        binfo.sum_s_q + m_block * kBlockM, n_block_max, n_block, n_block_chunked_continus_num, n_block_sink_token_mask_num, tVgV.data());
            }
        ...
        if (n_block > n_block_min) {
            // Advance gK
            // if(threadIdx.x==0){
            //     printf("#### tkgk n_block_max:%d, n_block:%d,  n_block_chunked_continus_num:%d, n_block_sink_token_mask_num:%d \n ",
            //             n_block_max, n_block, n_block_chunked_continus_num, n_block_sink_token_mask_num);
            // }
            if(n_block_max - n_block == (n_block_chunked_continus_num)){
                tKgK.data() = tKgK.data() + n_block_sink_token_mask_num * (-int(kBlockN * params.k_row_stride));
            }
    ...
    // [Note and Todo, wangboj] we assume all shink token don't need masking
    for (; n_block >= n_block_min; --n_block) {
        ...
        // Advance gV
        // if(threadIdx.x==0){
        //     printf("#### tvgv n_block_max:%d, n_block:%d,  n_block_chunked_continus_num:%d, n_block_sink_token_mask_num:%d \n ",
        //             n_block_max, n_block, n_block_chunked_continus_num, n_block_sink_token_mask_num);
        // }
        if(n_block_max - n_block == (n_block_chunked_continus_num + 1)){
            tVgV.data() = tVgV.data() + n_block_sink_token_mask_num*(-int(kBlockN * params.v_row_stride));
